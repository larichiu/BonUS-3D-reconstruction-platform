...
def metadata(row, data, settings):
    ...
    for field in ['mirror_operation','mirror_axis','image_width_px','anatomical_region','source_frame_path','source_mask_path']:
        if field in row:
            result[field] = str(row[field]) if field.endswith('_path') else row[field]
    return result
...
def v1_coordinate_export(points, data, settings):
    ...
    for frame_id, group in points.groupby('frame_index', sort=False):
        ...
        chunk = group.copy()
        if row.get('mirror_operation') == 'mirror':
            source_pixels = pixels.copy()
            source_pixels[:,0] = data.frame_size[0] - 1 - source_pixels[:,0]
            chunk['source_u'] = source_pixels[:,0]
            chunk['source_v'] = source_pixels[:,1]
            unmirrored = transform_pixels(source_pixels, new_transform, settings)
            for index, axis in enumerate('xyz'):
                chunk[f'unmirrored_{axis}_mm'] = unmirrored[:,index]
        ...
        if row.get('mirror_operation') == 'mirror':
            reflection = np.eye(4)
            reflection[0,0] = -1
            reflection[0,3] = data.frame_size[0] - 1
            scaling = np.diag([settings.spacing_x_mm, settings.spacing_y_mm, 1., 1.])
            source_to_output = new_transform @ scaling @ reflection
            record.update({f'SourcePixel_T_{i}{j}':source_to_output[i,j] for i in range(4) for j in range(4)})
