...
from variants.original.core.io import load_patient, write_ply, platform_revision
from variants.original.core.collection import annotated_sequences, load_collection
from variants.original.core.bone_display import add_bone_traces
from variants.original.core.models import ReconstructionSettings
from variants.original.core.geometry import rotation_from_wxyz
from variants.original.core.view import horizontal_scan_camera
from variants.original.core.frame_view import add_frame_plane
from variants.original.core.navigation import render_navigable_scene
from variants.original.core.reconstruct import load_binary_mask, probe_marker_pose, probe_pathway, reconstruct, statistical_filter, upper_boundary


st.set_page_config(page_title="Bone Reconstruction", page_icon="🦴", layout="wide")
st.title("Ultrasound bone reconstruction")
...
with st.sidebar:
    ...
    patient_name = st.selectbox("Patient", patient_names, index=default_patient, key="variantwidget_original_167")
    patient_folder = REDUCED_ROOT / patient_name
    combine_sequences = st.checkbox('Combine patient sequences with masks', value=False, key="variantwidget_original_169")
    selected_collection = []
    ...
    side_name = st.selectbox("Leg / side", side_names, index=side_names.index("right") if "right" in side_names else 0, disabled=combine_sequences, key="variantwidget_original_186")
    ...
    if not sequences:
        ...
        st.stop()
    sequence_name = st.selectbox("Sequence", [path.name for path in sequences], disabled=combine_sequences, key="variantwidget_original_193")
    ...
    reload_clicked = st.button("Reload selected sequence", width="stretch", key="variantwidget_original_197")
...
with tab_preview:
    ...
    if ready_indices:
        selected = st.select_slider("Preview matched frame", options=ready_indices, format_func=lambda i: frame_labels.get(i,str(i)), key="variantwidget_original_274")
...
with tab_geometry:
    ...
    c1, c2 = st.columns(2)
    with c1:
        spacing_x = st.number_input("Horizontal spacing (mm/pixel)", min_value=0.0001, value=0.05392, format="%.5f", key="variantwidget_original_293")
        spacing_y = st.number_input("Depth spacing (mm/pixel)", min_value=0.0001, value=0.05392, format="%.5f", key="variantwidget_original_294")
        sample_step = st.number_input("Boundary sample step (pixels)", min_value=1, value=2, step=1, key="variantwidget_original_295")
        frame_step = st.number_input("Use every Nth matched frame", min_value=1, value=1, step=1, key="variantwidget_original_296")
    with c2:
        st.caption("Probe marker → ultrasound tip offset (mm)")
        ox = st.number_input("Offset X", value=-163.326, key="variantwidget_original_299")
        oy = st.number_input("Offset Y", value=-19.6482, key="variantwidget_original_300")
        oz = st.number_input("Offset Z", value=-206.473, key="variantwidget_original_301")
...
with tab_reconstruct:
    display_mode = st.segmented_control(
     ...
     key="variantwidget_original_345") or "Both"
    show_bone = display_mode in {"Bone reconstruction", "Both"}
    show_pathway = display_mode in {"Probe pathway", "Both"}
    view_axes = st.radio('Display coordinate axes', ['NDI axes · stabilized at scan start', 'Leg-reference axes'], horizontal=True, key="variantwidget_original_353")
    horizontal_view = st.checkbox('Default view · probe left, reference marker right', value=True, key="variantwidget_original_354")
    reverse_view = st.checkbox('View from opposite side (camera only)', value=False, key="variantwidget_original_355")
    view_height = st.slider('3D view height', 600, 1400, 1000, 100, key="variantwidget_original_356")
    ...
    show_scene_labels = st.checkbox('Show text labels in 3D', value=False,
        help='Off keeps the view uncluttered. Hover over markers to identify them.', key="variantwidget_original_358")
    visibility = st.columns(2)
    show_accepted = visibility[0].checkbox('Accepted bone · solid sequence colour' if combine_sequences else 'Accepted bone · solid green', value=True, key="variantwidget_original_361")
    show_propagated = visibility[1].checkbox('Propagated bone · 5% opacity' if combine_sequences else 'Propagated bone · transparent orange', value=True, key="variantwidget_original_362")
    ...
    show_camera = st.checkbox(
     ...
     key="variantwidget_original_369")
    camera_frame = None
    if show_camera and valid_pose_indices:
        camera_frame = st.select_slider(
         ...
         key="variantwidget_original_375")

    if matched.any():
        minimum = int(matches.loc[matched, "FrameIndex"].min())
        maximum = int(matches.loc[matched, "FrameIndex"].max())
        frame_range = st.slider("Frame range", minimum, maximum, (minimum, maximum), key="variantwidget_original_383")
        if combine_sequences:
        ...
        frame_range = (0, 0)
    outlier_limit = st.slider("Robust outlier limit", 0.0, 10.0, 4.0, 0.5, help="0 disables filtering; lower values remove more distant points.", key="variantwidget_original_388")
    ...
    if st.button("Create bone reconstruction", type="primary", disabled=not matched.any(), key="variantwidget_original_394") or ((refresh_orientation or st.session_state.get('points') is None) and matched.any()):
    ...
    inspect_frames = st.checkbox('Inspect orientation · show a few original frames in 3D', value=False, key="variantwidget_original_410")
    ...
    if inspect_frames:
        ...
        image_count = st.slider('Number of original image planes', 1, max(2, min(20, len(choices))), min(3, max(2, len(choices))),
            help='Samples evenly across the sequence, rather than removing frames from the end.', key="variantwidget_original_415")
    ...
    if has_bone_points or has_path_points or not inspection_rows.empty:
        ...
        if st.checkbox('Show NDI tracker camera origin', value=True, key="variantwidget_original_529"):
        ...
        if has_path_points:
            ...
            st.download_button(
             ...
             key="variantwidget_original_591")
    ...
    if full_points is not None and not full_points.empty:
            ...
            st.download_button("Download bone CSV", points.to_csv(index=False).encode(), "bone_point_cloud.csv", "text/csv", key="variantwidget_original_598")
            ...
            st.download_button("Download bone PLY", ply_bytes, "bone_point_cloud.ply", "application/octet-stream", key="variantwidget_original_605")
