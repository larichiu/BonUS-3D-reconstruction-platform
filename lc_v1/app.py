...
from variants.lc_v1.core.calibration_export import v1_coordinate_export
from variants.lc_v1.core.sample_preview import sample_sequence_frames, sample_preview_figure
from variants.lc_v1.core.io import load_patient, write_ply, platform_revision
from variants.lc_v1.core.collection import annotated_sequences, load_collection
from variants.lc_v1.core.bone_display import add_bone_traces
from variants.lc_v1.core.models import ReconstructionSettings
from variants.lc_v1.core.geometry import rotation_from_wxyz
from variants.lc_v1.core.view import horizontal_scan_camera
from variants.lc_v1.core.frame_view import add_frame_plane
from variants.lc_v1.core.navigation import render_navigable_scene
from variants.lc_v1.core.reconstruct import load_binary_mask, probe_marker_pose, probe_pathway, reconstruct, statistical_filter, upper_boundary


st.set_page_config(page_title="Bone Reconstruction_LCedit_v1", page_icon="🦴", layout="wide")
st.title("Bone Reconstruction_LCedit_v1")
...
with st.sidebar:
    ...
    patient_name = st.selectbox("Patient", patient_names, index=default_patient, key="variantwidget_lc_v1_171")
    patient_folder = REDUCED_ROOT / patient_name
    combine_sequences = st.checkbox('Combine patient sequences with masks', value=False, key="variantwidget_lc_v1_173")
    selected_collection = []
    ...
    side_name = st.selectbox("Leg / side", side_names, index=side_names.index("right") if "right" in side_names else 0, disabled=combine_sequences, key="variantwidget_lc_v1_190")
    ...
    if not sequences:
        ...
        st.stop()
    sequence_name = st.selectbox("Sequence", [path.name for path in sequences], disabled=combine_sequences, key="variantwidget_lc_v1_197")
    ...
    reload_clicked = st.button("Reload selected sequence", width="stretch", key="variantwidget_lc_v1_201")
...
with tab_preview:
    ...
    if ready_indices:
        selected = st.select_slider("Preview matched frame", options=ready_indices, format_func=lambda i: frame_labels.get(i,str(i)), key="variantwidget_lc_v1_278")
...
with tab_geometry:
    with st.expander('Image spacing, probe offset, and baseline directions', expanded=False):
        ...
        c1, c2 = st.columns(2)
        with c1:
            spacing_x = st.number_input("Horizontal spacing (mm/pixel)", min_value=0.0001, value=0.05392, format="%.5f", key="variantwidget_lc_v1_298")
            spacing_y = st.number_input("Depth spacing (mm/pixel)", min_value=0.0001, value=0.05392, format="%.5f", key="variantwidget_lc_v1_299")
            sample_step = st.number_input("Boundary sample step (pixels)", min_value=1, value=2, step=1, key="variantwidget_lc_v1_300")
            frame_step = st.number_input("Use every Nth matched frame", min_value=1, value=1, step=1, key="variantwidget_lc_v1_301")
        with c2:
            st.caption("Probe marker → ultrasound tip offset (mm)")
            ox = st.number_input("Offset X", value=-163.326, key="variantwidget_lc_v1_304")
            oy = st.number_input("Offset Y", value=-19.6482, key="variantwidget_lc_v1_305")
            oz = st.number_input("Offset Z", value=-206.473, key="variantwidget_lc_v1_306")
    ...
    if st.button('Reset u and v to original orientation', key="variantwidget_lc_v1_323"):
...
with tab_geometry:
    ...
    show_original_outline = preview_columns[1].checkbox('Show original plane outlines for comparison', value=True, key="variantwidget_lc_v1_356")
...
with tab_reconstruct:
    display_mode = st.segmented_control(
     ...
     key="variantwidget_lc_v1_376") or "Both"
    show_bone = display_mode in {"Bone reconstruction", "Both"}
    show_pathway = display_mode in {"Probe pathway", "Both"}
    view_axes = st.radio('Display coordinate axes', ['NDI axes · stabilized at scan start', 'Leg-reference axes'], horizontal=True, key="variantwidget_lc_v1_384")
    horizontal_view = st.checkbox('Default view · probe left, reference marker right', value=True, key="variantwidget_lc_v1_385")
    reverse_view = st.checkbox('View from opposite side (camera only)', value=False, key="variantwidget_lc_v1_386")
    view_height = st.slider('3D view height', 600, 1400, 1000, 100, key="variantwidget_lc_v1_387")
    ...
    show_scene_labels = st.checkbox('Show text labels in 3D', value=False,
        help='Off keeps the view uncluttered. Hover over markers to identify them.', key="variantwidget_lc_v1_389")
    visibility = st.columns(2)
    show_accepted = visibility[0].checkbox('Accepted bone · solid sequence colour' if combine_sequences else 'Accepted bone · solid green', value=True, key="variantwidget_lc_v1_392")
    show_propagated = visibility[1].checkbox('Propagated bone · 5% opacity' if combine_sequences else 'Propagated bone · transparent orange', value=True, key="variantwidget_lc_v1_393")
    ...
    if show_camera and valid_pose_indices:
        camera_frame = st.select_slider(
         ...
         key="variantwidget_lc_v1_406")

    if matched.any():
        minimum = int(matches.loc[matched, "FrameIndex"].min())
        maximum = int(matches.loc[matched, "FrameIndex"].max())
        frame_range = st.slider("Frame range", minimum, maximum, (minimum, maximum), key="variantwidget_lc_v1_414")
        if combine_sequences:
        ...
        frame_range = (0, 0)
    outlier_limit = st.slider("Robust outlier limit", 0.0, 10.0, 4.0, 0.5, help="0 disables filtering; lower values remove more distant points.", key="variantwidget_lc_v1_419")
    ...
    if st.button("Create bone reconstruction", type="primary", disabled=not matched.any(), key="variantwidget_lc_v1_425") or ((refresh_orientation or st.session_state.get('points') is None) and matched.any()):
    ...
    inspect_frames = st.checkbox('Inspect orientation · show a few original frames in 3D', value=False, key="variantwidget_lc_v1_441")
    ...
    if inspect_frames:
        ...
        image_count = st.slider('Number of original image planes', 1, max(2, min(20, len(choices))), min(3, max(2, len(choices))),
            help='Samples evenly across the sequence, rather than removing frames from the end.', key="variantwidget_lc_v1_446")
    ...
    if has_bone_points or has_path_points or not inspection_rows.empty:
        ...
        if st.checkbox('Show NDI tracker camera origin', value=True, key="variantwidget_lc_v1_560"):
    ...
    if full_points is not None and not full_points.empty:
            ...
            st.caption('CSV includes original XYZ (u=v=0), new XYZ, and T_00…T_33. T maps [pixel_u × spacing_u_mm, pixel_v × spacing_v_mm, 0, 1] to output XYZ in millimetres; matrix entries use row-major order and column vectors. x/y/z match new_x/y/z. Original and new positions use the same currently selected geometry and pixel samples; only the added rotations differ.')
            if st.button('Build calibrated CSV exports', key="variantwidget_lc_v1_631"):
            ...
            if csv_export is not None and csv_export[0] == export_key:
                st.download_button('Download bone CSV · original and new XYZ', csv_export[1], 'Bone_Reconstruction_LCedit_v1_coordinates.csv', 'text/csv', key="variantwidget_lc_v1_637")
                st.download_button('Download transformation matrices CSV · one per frame', csv_export[2], 'Bone_Reconstruction_LCedit_v1_matrices.csv', 'text/csv', key="variantwidget_lc_v1_638")
            ...
            st.download_button("Download bone PLY", ply_bytes, "bone_point_cloud.ply", "application/octet-stream", key="variantwidget_lc_v1_645")
